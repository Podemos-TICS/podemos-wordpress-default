...
function bootstrapwp_widgets_init() {
    ...
    register_sidebar(
            array(
                'name' => __('Home Left', 'podemoswp'),
                'id' => 'home-left',
                'description' => __('Left textbox on homepage', 'podemoswp'),
    ...
    register_sidebar(
            array(
                'name' => __('Home Middle', 'podemoswp'),
                'id' => 'home-middle',
                'description' => __('Middle textbox on homepage', 'podemoswp'),
                'before_widget' => '<div id="%1$s" class="widget %2$s">',
                'after_widget' => '</div>',
                'before_title' => '<h2>',
                'after_title' => '</h2>'
    ...
    register_sidebar(
            array(
                'name' => __('Home Right', 'podemoswp'),
                'id' => 'home-right',
                'description' => __('Right textbox on homepage', 'podemoswp'),
                'before_widget' => '<div id="%1$s" class="widget %2$s">',
                'after_widget' => '</div>',
                'before_title' => '<h2>',
                'after_title' => '</h2>'
...
if (!function_exists('bootstrapwp_content_nav')):
    function bootstrapwp_content_nav($nav_id) {
        ...
        <nav id="<?php echo $nav_id; ?>" class="navigation" role="navigation">
            <h3 class="assistive-text"><?php _e('Post navigation', 'podemoswp'); ?></h3>
            <div class="nav-previous alignleft"><?php next_posts_link(
                __('<span class="meta-nav">&larr;</span> Older posts', 'podemoswp')
            ); ?></div>
            <div class="nav-next alignright"><?php previous_posts_link(
                __('Newer posts <span class="meta-nav">&rarr;</span>', 'podemoswp')
            ); ?></div>
        ...
        switch ($comment->comment_type) :
            ...
            case 'trackback' :
                ...
                ?>
                    ...
                    _e(
                            'Your comment is awaiting moderation.', 'podemoswp'
                    );
                    ?></p>
                            <?php endif; ?>
                        ...
                        printf('<a href="%1$s"><time datetime="%2$s">%3$s</time></a>', esc_url(get_comment_link($comment->comment_ID)), get_comment_time('c'), sprintf(
                                        __('%1$s at %2$s', 'podemoswp'), get_comment_date(), get_comment_time()
                                )
                        );
                        ?>
                        </p>
                        <p class="reply">
                            ...
                            comment_reply_link(array_merge($args, array(
                                'reply_text' => __('Reply <span>&darr;</span>', 'podemoswp'),
                                'depth' => $depth,
...
if (!function_exists('bootstrapwp_posted_on')) :
	...
	$options = get_option('podemoswp_theme_options');
        ...
        if($options['meta_data'] == true ){
              ...
              printf(__('Posted on <a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a><span class="byline"> <span class="sep"> by </span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'podemoswp'), esc_url(get_permalink()), esc_attr(get_the_time()), esc_attr(get_the_date('c')), esc_html(get_the_date()), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_attr(sprintf(__('View all posts by %s', 'podemoswp'), get_the_author())), esc_html(get_the_author())
        );
...
function bootstrapwp_wp_title($title, $sep) {
    ...
    if ($paged >= 2 || $page >= 2) {
        $title = "$title $sep " . sprintf(__('Page %s', 'podemoswp'), max($paged, $page));
    }
    return $title;
}
...
function bootstrapwp_breadcrumbs() {
    ...
    if (!is_home() && !is_front_page() || is_paged()) {
        ...
        if (is_category()) {
            ...
            echo $before . __('Search results for "', 'podemoswp') . get_search_query() . '"' . $after;
        } elseif (is_tag()) {
            echo $before . __('Posts tagged "', 'podemoswp') . single_tag_title('', false) . '"' . $after;
        } elseif (is_author()) {
            global $author;
            $userdata = get_userdata($author);
            echo $before . __('Articles posted by ', 'podemoswp') . $userdata->display_name . $after;
        } elseif (is_404()) {
            echo $before . __('Error 404', 'podemoswp') . $after;
        }
